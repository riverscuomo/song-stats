...
def ensure_required_headers(sheet, methods):
    ...
    try:
        ...
        required_headers = {
            ...
            'spotify': ['track_id', 'song_popularity', 'duration', 'tempo_spotify', 'energy',  'artist_id', 'year'], # 'danceability', 
        ...
        if missing_headers:
            ...
            logger.info(
                f"Added {len(missing_headers)} missing headers to the sheet")
            return True
        else:
...
def process_spreadsheet(spreadsheet_name, worksheet_name, methods, start_row=0, config=None):
    ...
    """
    logger.info(
        f"Processing spreadsheet: {spreadsheet_name}, worksheet: {worksheet_name}")
    ...
    try:
        ...
        try:
            ...
            logger.error(
                "Check your GOOGLE_SHEETS_CREDENTIALS_FILE path and file contents")
            return False
        ...
        if "spotify" in methods:
            try:
                ...
                logger.error(
                    f"Failed to connect to Spotify API: {spotify_error}")
                spotify_client = None
        ...
        if "youtube" in methods:
            try:
                ...
                logger.error(
                    f"Failed to connect to YouTube API: {youtube_error}")
                youtube_client = None
        ...
        if "lyrics" in methods:
            try:
                ...
                logger.error(
                    f"Failed to connect to Genius API: {genius_error}")
                genius_client = None
    ...
    try:
        # Get the sheet
        logger.info(
            f"Attempting to access Google Sheet: '{spreadsheet_name}', worksheet: '{worksheet_name}'")
        try:
            ...
            logger.error(
                "Check if the spreadsheet exists and is shared with your Google service account")
            return False
        ...
        try:
            ...
            logger.info(
                f"Retrieved data from sheet (raw row count: {len(data)})")
        ...
        if start_row > 0:
            data = data[start_row:]
            logger.info(
                f"Starting from row {start_row}, {len(data)} rows remaining")
        ...
        for row in data[:1]:  # Check just the first row
            if 'artist_name' not in row or 'song_title' not in row:
                logger.error(
                    "Missing required columns! Sheet must have 'artist_name' and 'song_title' columns.")
                return False
        ...
        for i, row in enumerate(data):
            logger.info(
                f"Processing row {i+start_row+2}: {row.get('artist_name', '')} - {row.get('song_title', '')}")

            # Check if required fields exist
            if not row.get('artist_name') or not row.get('song_title'):
                logger.warning(
                    f"Missing artist_name or song_title in row {i+start_row+2}, skipping")
                continue
        ...
        if updated:
            ...
            # +2 for 1-based index and header row
            update_range(sheet, data, start_row=start_row+2)
...
def update_spotify_data(row, spotify_client):
    ...
    try:
        ...
        if not song_data:
            logger.warning(
                f"No Spotify data found for {artist_name} - {song_title}")
            return False
        ...
        row['duration'] = int(song_data.get(
            'duration_ms', 0) / 1000)  # Convert to seconds
        ...
        if 'genres' in row and not row.get('genres'):
            # Format as string like original
            row['genres'] = str(song_data.get('genres', []))
...
def update_youtube_data(row, youtube_client):
    ...
    try:
        ...
        if row.get('cover', '').lower() == 'x':
            logger.info(
                f"Skipping YouTube data for cover song: {artist_name} - {song_title}")
            row['youtube_views'] = ""
            return False

        # Get view count from YouTube
        view_count = get_video_view_count(
            artist_name, song_title, youtube_client)

        if view_count > 0:
            # Use youtube_views field name as in original script
            row['youtube_views'] = view_count
            logger.info(
                f"Updated YouTube views for {song_title}: {view_count:,}")
            return True
        else:
            logger.warning(
                f"No YouTube data found for {artist_name} - {song_title}")
            return False
    except Exception as e:
...
def update_lyrics_data(row, genius_client):
    ...
    try:
        ...
        if row.get('lyrics') not in ["", "!", None]:
            logger.info(
                f"Lyrics already exist for {artist_name} - {song_title}, skipping")
            return False

        # Get lyrics from Genius
        logger.info(
            f"Looking up lyrics for {artist_name} - {song_title} on Genius")
        ...
        if lyrics:
            ...
            row['lyrics'] = lyrics
            logger.info(
                f"Found lyrics for {artist_name} - {song_title} ({len(lyrics)} characters)")
            return True
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Collect and analyze song data')
